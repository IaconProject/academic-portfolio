...
interface PublicationsEditorProps {
  ...
  onSave: (updatedPubs: PublicationItem[]) => void;
}
...
export const PublicationsEditor: React.FC<PublicationsEditorProps> = ({ publications, onSave }) => {
  ...
  const handleSaveItem = () => {
    if (!formState.title || !formState.year) return;

    if (editingId === 'new') {
      const newItem: PublicationItem = {
        id: `pub-${Date.now()}`,
        type: formState.type || 'Makale',
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-stone-100 dark:border-stone-800 pb-4">
        <div>
          <h2 className="text-xl font-bold text-stone-900 dark:text-stone-100 tracking-tight flex items-center gap-2">
            ...
            <span>Akademik Yayınlar Yönetimi</span>
          </h2>
          <p className="text-xs text-stone-500 dark:text-stone-400 mt-1">
            Makale, kitap bölümü, bildiri ve araştırma yayınlarınızı yönetin.
          </p>
        </div>
        <button
          onClick={startAdd}
      ...
      {editingId && (
        <div className="p-5 bg-stone-50 dark:bg-stone-800/80 border border-stone-200 dark:border-stone-700 rounded-xl space-y-4">
          <h3 className="text-xs font-bold uppercase tracking-wider text-stone-700 dark:text-stone-300 border-b border-stone-200 dark:border-stone-700 pb-2">
            {editingId === 'new' ? 'Yeni Yayın Kaydı' : 'Yayın Kaydını Düzenle'}
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-stone-700 dark:text-stone-300 mb-1">Yayın Türü</label>
              <select
                value={formState.type || 'Makale'}
              ...
              >
                ...
                <option value="Bildiri">Bildiri / Sempozyum</option>
                <option value="Tez">Akademik Tez</option>
                <option value="Diğer">Diğer</option>
              </select>
            </div>
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-stone-700 dark:text-stone-300 mb-1">Yayımlandığı Yıl</label>
              <input
                type="text"
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-bold uppercase tracking-wider text-stone-700 dark:text-stone-300 mb-1">Yayın Başlığı</label>
              <input
                type="text"
                placeholder="Yayın tam adını girin..."
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-stone-700 dark:text-stone-300 mb-1">Yayıncı / Dergi Adı</label>
              <input
                type="text"
                placeholder="Örn: İslam Hukuku Araştırmaları Dergisi"
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-stone-700 dark:text-stone-300 mb-1">DOI veya Yayın Bağlantısı (URL)</label>
              <input
                type="text"
                placeholder="https://doi.org/... veya https://..."
                value={formState.url || formState.doi || ''}
                onChange={(e) => setFormState({ ...formState, url: e.target.value, doi: e.target.value })}
                className="w-full px-3.5 py-2.5 bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-xl text-sm text-stone-900 dark:text-stone-100 focus:border-stone-900 dark:focus:border-amber-400 outline-none"
              />
            </div>
          </div>
        ...
        {items.map((item) => (
          ...
          >
            <div>
              ...
              {item.publisher && <p className="text-xs text-stone-500 dark:text-stone-400 mt-1">{item.publisher}</p>}
            </div>
