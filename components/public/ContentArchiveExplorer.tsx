              ...
              <div className="flex flex-1 flex-col p-5 sm:p-6">
                ...
                {!!item.tags.length && (
                  <ul aria-label="Etiketler" className="mt-4 flex flex-wrap gap-1.5">
                    {item.tags.slice(0, 5).map((tag) => <li key={tag} className="rounded-lg bg-academic-surface-muted px-2.5 py-1 text-[11px] font-semibold text-academic-slate">{tag}</li>)}
                  </ul>
                )}
                <div className="mt-auto flex flex-wrap items-center gap-4 pt-6 text-xs font-black">
                  {item.href && <Link href={item.href} className="inline-flex items-center gap-1.5 text-academic-ink hover:underline">Detayı oku <ArrowUpRight className="h-3.5 w-3.5" /></Link>}
...
function EmptyState({ title, description }: { title: string; description: string }) {
  return (
    <div className="rounded-3xl border border-dashed border-academic-border bg-academic-surface p-10 text-center sm:p-14">
      <Search className="mx-auto h-8 w-8 text-academic-slate/60" />
